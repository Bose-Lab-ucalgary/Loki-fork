        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'coattn':
                    ...
                    # pzhang modified
                    path_features = torch.load(os.path.join(data_dir, 'pt_files',
                                                            'weighted_image_{}.pt'.format(case_id)))
                    omic_features = torch.load(os.path.join(data_dir, 'pt_files',
                                                            'text_{}.pt'.format(case_id)))
